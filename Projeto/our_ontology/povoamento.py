...
query_locais = """
SELECT ?pais ?paisLabel ?paisArea ?paisPop ?capital ?capitalLabel ?capitalArea ?capitalPop ?continente ?continenteLabel ?continenteArea ?continentePop
WHERE {
  ?pais wdt:P31 wd:Q6256.             # País
  ?pais wdt:P36 ?capital.             # Capital
  ?pais wdt:P30 ?continente.          # Continente

  OPTIONAL { ?pais wdt:P2046 ?paisArea. }         # Área do país
  OPTIONAL { ?pais wdt:P1082 ?paisPop. }          # População do país

  OPTIONAL { ?capital wdt:P2046 ?capitalArea. }   # Área da capital
  OPTIONAL { ?capital wdt:P1082 ?capitalPop. }    # População da capital

  OPTIONAL { ?continente wdt:P2046 ?continenteArea. } # Área do continente
  OPTIONAL { ?continente wdt:P1082 ?continentePop. }  # População do continente
...
for item in data["results"]["bindings"]:
    ...
    if id_pais not in paises:
        ...
        if "paisArea" in item:
            paisArea = item["paisArea"]["value"]
            g.add((paisURI, n.área, Literal(paisArea)))
        if "paisPop" in item:
            paisPop = item["paisPop"]["value"]
            g.add((paisURI, n.população, Literal(paisPop)))
        ...
        if continente not in continentes:
            ...
            if "continenteArea" in item:
                continenteArea = item["continenteArea"]["value"]
                g.add((continenteURI, n.área, Literal(continenteArea)))
            if "continentePop" in item:
                continentePop = item["continentePop"]["value"]
                g.add((continenteURI, n.população, Literal(continentePop)))
        else:
        ...
        if id_capital not in capitais:
            ...
            if "capitalArea" in item:
                capitalArea = item["capitalArea"]["value"]
                g.add((capitalURI, n.área, Literal(capitalArea)))
            if "capitalPop" in item:
                capitalPop = item["capitalPop"]["value"]
                g.add((capitalURI, n.população, Literal(capitalPop)))
        else:
...
query_oceanos = """
SELECT ?oceano ?oceanoLabel ?area WHERE {
  ?oceano wdt:P31 wd:Q9430. # instância de oceano
  OPTIONAL { ?oceano wdt:P2046 ?area. }
...
for item in data["results"]["bindings"]:
    ...
    if id_oceano not in oceanos:
        ...
        if "area" in item:
            ...
            g.add((oceanoURI, n.área, Literal(area)))
...
for item in data["results"]["bindings"]:
    ...
    marURI = add_mar(id_mar, mar, area)
...
query_corpos_agua = """
SELECT ?mar ?marLabel ?area ?oceano WHERE {
  ?mar wdt:P31 wd:Q165. 
  ?mar wdt:P361 ?oceano. 
  ?oceano wdt:P31 wd:Q9430.
  ...
  ?oceano rdfs:label ?oceanoLabel.
  FILTER(LANG(?oceanoLabel) = "pt")
}
"""

response = requests.get(endpoint_url, params={'query': query_corpos_agua}, headers=headers)
data = response.json()
...
for item in data["results"]["bindings"]:
    ...
    if id_oceano not in oceanos:
        continue  # Skip if the sea or ocean is not in our data
    
    oceanoURI = oceanos[id_oceano]

    marURI = add_mar(id_mar, mar, area)

    g.add((marURI, n.éParteDe, oceanoURI))
    g.add((oceanoURI, n.temMar, marURI))
...
query_rios = """
SELECT ?rio ?rioLabel ?pais ?desagua ?desaguaLabel ?comprimento WHERE {
  ?rio wdt:P31 wd:Q4022.
  ?rio wdt:P17 ?pais.
  ?rio wdt:P403 ?desagua.
  OPTIONAL { ?rio wdt:P2043 ?comprimento. }
...
for item in data["results"]["bindings"]:
    ...
    if id_rio not in rios:
        ...
        if "comprimento" in item:
            comprimento = item["comprimento"]["value"]
            g.add((rioURI, n.comprimento, Literal(comprimento)))
    else:
        rioURI = rios[id_rio]
...
open("geografia.ttl", "w").write(final_ttl)